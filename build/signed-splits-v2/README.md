...
- `com.somewearlabs.swtak.plugin.apk` — gateway v18 base package containing `SomewearGatewayProvider`, the application-level Realm bootstrap, retained Somewear runtime, controlled Radio-to-Satellite fallback, durable receiver-requested SC3 Radio fragmentation, peer completion acknowledgement, durable completed-message replay with explicit local acknowledgement, router/service resubscription diagnostics, acknowledged native Somewear Satellite composite transport, legacy v14 Satellite-frame receive compatibility, API-v2 workspace invite enrollment, Node telemetry/settings, mesh status, message cancellation, power, cloud-backed file/image bridges, authenticated workspace file catalogue, signature-protected QR scanner, and bound receive-lifetime service.
