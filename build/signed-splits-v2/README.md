...
- `com.somewearlabs.swtak.plugin.apk` — gateway v16 base package containing `SomewearGatewayProvider`, the application-level Realm bootstrap, retained Somewear runtime, controlled Radio-to-Satellite fallback, SC3 Radio fragmentation, acknowledged native Somewear Satellite composite transport, legacy v14 Satellite-frame receive compatibility, API-v2 workspace invite enrollment, Node telemetry/settings, mesh status, message cancellation, power, cloud-backed file/image bridges, authenticated workspace file catalogue, signature-protected QR scanner, and bound receive-lifetime service.
