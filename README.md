...
Gateway API v2 passed bidirectional Android contract tests for initialization, explicit routing, inbound polling/Flow delivery, message-ID preservation, delivery lookup, cancellation, and controlled Radio-to-Satellite handover. Gateway v18 adds a durable completed-message inbox, cumulative local acknowledgement, automatic router/service resubscription, and receive-chain diagnostics while retaining v17's receiver-requested Radio fragment recovery, peer completion acknowledgement, and revisioned variable-size file batches. On Android, a 2.3 KB CAS JSON survived a forced gateway-process restart, replayed with the same ID/content/sequence, and disappeared only after explicit acknowledgement; the health snapshot remained subscribed with zero drops/errors. Oversized Satellite sends still remain one parent `MessagePayload`, allowing the retained Somewear `CompositePackager` and `PostOffice` to split and reassemble native `Part` records. Every Satellite send requires the retained backhaul acknowledgement flag, so Node handoff alone is not reported as sufficient. SC3 framing remains enabled only for Radio, where the retained core explicitly does not accept composite children. The API also exposes the authenticated workspace file catalogue and SDK-owned selective recovery/cache, alongside Node/satellite telemetry, mesh topology and signal quality, hardware settings, workspace QR enrollment, file/image transfer, and receive-health diagnostics.
